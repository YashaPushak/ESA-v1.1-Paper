...
\author{
Yasha Pushak\inst{1}, Zongxu Mu\inst{1}
\and Holger Hoos\inst{2,1}
}
\institute{
%  \orgname{Department of Computer Science,
%  The University of British Columbia}, \cny{Canada} \and
	Department of Computer Science,
	The University of British Columbia, Canada \and
  LIACS, Universiteit Leiden, The Netherlands \\ \medskip
  \email{\{ypushak,zongxumu\}@cs.ubc.ca, hh@liacs.nl} 
}
...
% 1. AI Communications
% 2. International Journal on Artificial Intelligence Tools
...
%YP: CUrrently 140 words -- fits within the 150 word limit for AI Communications.
\end{abstract}
...
Very few methods exist for performing empirical running time scaling analysis that handle noise and stochastic behaviour of algorithms in a principled, statistical way~\cite{Hoo09}. 
...
In slightly more advanced work, standard least squares regression and curve-fitting procedures are used to fit and subsequently visualise  trend lines~\cite{ZapHau12}. 
...
A significant advance in empirical running time scaling analysis was achieved by Hoos~\cite{Hoo09}, who introduced a new methodology to address both shortcomings by using
%HH: (TODO) You should slightly reword above to make it very clear to the reader what the two shortcomings are.
a complex bootstrap sampling procedure to handle variability in running time in a statistically meaningful way, and by assessing extrapolation accuracy using a set of ``challenge'' instances withheld during the model fitting.  
...
\caption{\label{fig:methodology}Empirical scaling analysis approach underlying ESA.}
\end{figure}
...
\caption{\label{fig:ESA-auto-interpretation} Procedure used by ESA for automatic interpretation of scaling analysis results. Details on the conditions are provided in the main text.}
\end{figure}
...
\end{lstlisting}
\vspace*{-3mm}
\caption{\label{fig:Excerpt-ESA-input}Example input file for ESA, where ``...'' represents omitted lines analogous to those shown.}
\end{figure}
...
\caption{\label{fig:ESA-fitted-models}Example output of ESA -- 
running times, fitted models and corresponding bootstrap confidence
intervals.}
\end{figure}
...
\hline 
\multirow{3}{*}{EAX (default)} & Exp. & $1.0888$ & $\left[1.0316,1.143\right]$ & $1548$ & $\left[1406.7,1693.6\right]$ \tabularnewline 
...
other for challenge data, as illustrated in Table \ref{tab:ESA-bootstrap-support}.
\begin{table}[t]
\begin{centering}
...
\begin{centering}
\caption{Observed and predicted intervals example}
...
Example output of ESA -- bootstrap
confidence intervals for observed running times and predictions from the fitted root-exponential model for the support data (upper table) and the challenge data (lower table); ``...'' represents omitted lines analogous to those shown.
\end{table}
...
\caption{\label{fig:Snapshot-ESA-output}A snapshot of the 7-page technical report generated
by ESA.}
\end{figure}
...
In the following, we provide only a high-level summary of our findings, and distill from these results our advice on best-practices for using ESA. For a substantially more detailed discussion and presentation of the results, please see our online, supplementary material~\todo{Move online. Currently available at https://www.overleaf.com/6216557917nztjdzchjhxp }
...
\caption{\label{fig:Fitted-models-LKH} Scaling models for median 
running times of LKH for finding optimal solutions to RUE instances.
}
\end{figure}
\begin{figure}[t]
\noindent \begin{centering}
...
\caption{\label{fig:Fitted-models-concorde} Scaling models for the medians
of the running times for Concorde to find optimal solutions of RUE
instances. }
\end{figure}
...
% \todo{Did Zongxu have funding that needs to be added here? HH: No.}
