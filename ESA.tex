...
We believe that this is caused by outliers in the data for large support instances sizes that lead to some model overfitting within ESA.
...
In our example, we can see that this is the case by examining the smallest support instance sizes, for which the curvature of the square-root exponential model is beginning to pull the model above the observed running times.
%HH: <- also point to residual plots? It might even be worth including one ... (TODO)
Similar situations may occure in other scenarios where the best-fit model may not accurately capture the asymptotic scaling due to the effects of lower-order terms. 
...
%HH: <- reworded this; make sure it is still true (TODO)
