...
Our interpretation procedure especially emphasizes the challenge points for larger instance sizes, as those provide more information regarding whether the model predictions scale with instance size. The detailed criteria, which we 
designed based on extensive experiments with several algorithms (both real and artificial), are as follows:
%HH: I've made some changes in the following, in light of the fact that we technically define consistency etc. for predictions, not for intervals (although we do it based on confidence intervals) - TODO: check
\begin{itemize}
...
\item \textbf{fair fit}: the model predicts well for most of the challenge sizes; more precisely, $\geq 90\%$ of the predictions for challenge sizes or $\geq 90\%$ of predicions for the larger half of the challenge sizes are weakly consistent;
...
%HH: Missing: some explanation why these classes were defined in this way - TODO: add brief explanation
