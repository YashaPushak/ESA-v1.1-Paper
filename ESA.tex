...
BC, \cny{Canada}%\printead[presep={\\}]{}
}
\address[B]{LIACS, \orgname{Universiteit Leiden}, \cny{The Netherlands}\printead[presep={\\}]{e1,e2,e3}}
...
\hline
\# instances & 1000 & 1000 & $\cdots$ & 1000 \tabularnewline
\# running times & 1000 & 1000 & $\cdots$ & 1000 \tabularnewline
mean & $1.901$ & $2.951$ & $\cdots$ & $29.15$ \tabularnewline
coefficient of variation & $0.4569$ & $1.05$ & $\cdots$ & $6.151$ \tabularnewline
Q(0.1) & $1.495$ & $2.111$ & $\cdots$ & $10.84$ \tabularnewline
Q(0.25) & $1.601$ & $2.253$ & $\cdots$ & $11.58$ \tabularnewline
median & $1.73$ & $2.516$ & $\cdots$ & $13.21$ \tabularnewline
Q(0.75) & $1.917$ & $2.774$ & $\cdots$ &  $24.97$ \tabularnewline
Q(0.9) & $2.08$ & $3.181$ & $\cdots$ & $34.69$ \tabularnewline
\hline
\end{tabular}}\medskip{}
\par
\end{centering}
...
\begin{table}[t]
\caption{Individual scaling models fitted to EAX on RUE Instances \label{tab:ESA-fitted-models}}
\begin{centering}
\scalebox{0.75}{%
\begin{tabular}{ccccc}
\hline
...
\hline
\hline
\multirow{3}{*}{EAX} & Exp. Model & $1.0511\times 1.0017^{x}$ & $1.1903$ & $\left[674.26,864.85\right]$ \tabularnewline
 & RootExp. Model & $\mathbf{0.15777\times 1.1215^{\sqrt{x}}}$ & $\mathbf{0.22553}$ & $\mathbf{\left[24.586,183.39\right]}$ \tabularnewline
 & Poly. Model & $1.409\times10^{-5}\times x^{1.8788}$ & $0.11865$ & $\left[122.17,300.04\right]$ \tabularnewline
\hline
\end{tabular}}%
\medskip{}
\par
...
\hline
\multirow{3}{*}{EAX} & Exp. & $\left[1.0177,1.079\right]$ & $\left[1.0017,1.0017\right]$ \tabularnewline
 & RootExp. & $\left[0.14835,0.16531\right]$ & $\left[1.1198,1.1236\right]$ \tabularnewline
 & Poly. & $\left[1.1187\times10^{-5},1.6711\times10^{-5}\right]$ & $\left[1.8542,1.9122\right]$ \tabularnewline
\hline
\end{tabular}%
}\medskip{}
...
\begin{table}[t]
\begin{centering}
\caption{Bootstrap confidence intervals for RMSE of EAX scaling models \label{tab:ESA-bootstrap-RMSE}}
\scalebox{0.75}{%
...
\hline
\multirow{3}{*}{EAX} & Exp. & $0.45758$ & $\left[0.40345,0.51484\right]$ & $780.07$ & $\left[417.88,980.96\right]$ \tabularnewline
 & RootExp. & $\mathbf{0.20977}$ & $\mathbf{\left[0.16623,0.28264\right]}$ & $\mathbf{82.313}$ & $\mathbf{\left[11.721,468.78\right]}$ \tabularnewline
 & Poly. & $0.21901$ & $\left[0.15746,0.28357\right]$ & $200.36$ & $\left[95.001,574.23\right]$ \tabularnewline
\hline
\end{tabular}%
}\medskip{}
...
\begin{table*}[t]
\begin{centering}
\caption{Confidence intervals for observed and predicted challenge data}
\label{tab:ESA-bootstrap-challenge}
\scalebox{0.75}{%
\begin{tabular}{ccccc}
\hline
\multirow{2}{*}{Solver} & \multirow{2}{*}{$n$} & Prediction confidence intervals & \multicolumn{2}{c}{Observed median run-time}\tabularnewline
 &  & RootExp. model  & Point estimates  & Confidence intervals\tabularnewline
\hline
\hline
\multirow{4}{*}{EAX} & 2000 & $\mathbf{\left[26.06,27.26\right]}$ & $26.19$ & $\left[25.3,27.92\right]$ \tabularnewline
 & 2500 & $\mathbf{\left[47.37,50.44\right]}$ & $46.82$ & $\left[39.03,68.04\right]$ \tabularnewline
 & $\cdots$ & $\cdots$ & $\cdots$ & $\cdots$  \tabularnewline
 & 4500 & $\mathbf{\left[327.5,369.2\right]}$ & $\left[303.3,766.8\right]$ & $\left[207.5,1482\right]$ \tabularnewline
\hline
\end{tabular}%
}\medskip{}
...
%HH: <- Done: put on-line before submission
...
In another application of the ESA methodology, Dubois \etal{} studied the scaling for the state-of-the-art inexact TSP solvers, EAX~\cite{NagKob13} and LKH~\cite{helsgaun2000effective} on the same set of RUE instances~\cite{DubEtAl15}.
%. However, since EAX and LKH are inexact solvers, they cannot prove the optimality of the solutions they find, and hence we recorded the running times required to find the optimal solution qualities previously found using Concorde. Unfortunately, some of the larger problem instances remained unsolved even with long runs of Concorde, which added additional uncertainty into the scaling analysis.
...
%HH: <- Yasha, please fill in degree of polynomial (DONE)
%We studied two versions of LKH (1.3 and 2) and we found that version 1.3 appeared to scale as a square-root exponential model, while version 2 appeared to have observed scaling between a square-root exponential model and a polynomial model. 
...
%HH: <- Yasha, please check whether LKH 2 scaling with between root-exp and poly; if so, indicate here, including degree of poly (Done)
...
However, while this enhancement augments the previous scaling analysis results and provides a well-defined, principled and rigorous way for assessing the scaling models fitted and assessed by ESA, it does not change any of the previous results. 
...
%HH: <- It might be good to say what the new model says when run on some of the data from previous experiments. Even a single example would help. (DONE)
